...
const EDIT_HEADER_ACTIONS = [
  { label: "Cancel", variant: "secondary" },
  { label: "Save Changes", variant: "primary" },
];

const EDIT_DEFAULT_FORM = {
  shelterName: "",
  description: "",
  logoUrl: "",
  operatingHoursStart: "",
...
export default function EditShelter() {
  ...
  const supabaseBucket =
    import.meta.env.VITE_SUPABASE_BUCKET || "public-images";
  ...
  useEffect(() => {
    ...
    async function fetchShelter() {
      ...
      try {
        ...
        const shelter = res?.data?.data || res?.data || {};
        const coordinates = Array.isArray(shelter?.location?.coordinates)
          ? shelter.location.coordinates
          : [];
        const [lng, lat] = coordinates;
        const ownerNgoId = shelter?.owner_ngo?._id || shelter?.owner_ngo || "";
        const fetchedCapacity = Number(shelter?.capacity);
        const safeCapacity =
          Number.isFinite(fetchedCapacity) && fetchedCapacity > 0
            ? fetchedCapacity
            : 0;
        const fetchedAvailability = Number(shelter?.availability);
        const safeAvailability = Number.isFinite(fetchedAvailability)
          ? Math.min(Math.max(fetchedAvailability, 0), safeCapacity)
          : 0;
        initialOccupiedBedsRef.current = Math.max(
          safeCapacity - safeAvailability,
          0,
        );

        if (!isMounted) return;

        const logoUrl = Array.isArray(shelter?.images) ? shelter.images[0] : "";
        const intakeHours = shelter?.intake_hours || {};
        const facilities = Array.isArray(shelter?.facilities)
          ? shelter.facilities
          : [];

        setForm((prev) => ({
          ...prev,
          shelterName: shelter?.address?.building || prev.shelterName,
          description: shelter?.requirements || "",
          logoUrl,
          operatingHoursStart: intakeHours?.startTime || "",
          operatingHoursEnd: intakeHours?.endTime || "",
          operatingHoursEmergencyInterval:
            intakeHours?.emergency_interval || "",
          totalBeds: shelter?.capacity ?? prev.totalBeds,
          phone: shelter?.phone_number || "",
          email: shelter?.email || "",
          website: shelter?.website || "",
          streetAddress: shelter?.address?.street || "",
          city: shelter?.address?.city || "",
          ownerNgo: getOwnerNgoId(ownerNgoId),
          lng: lng != null ? String(lng) : "",
          lat: lat != null ? String(lat) : "",
          facilities: facilities.length ? facilities : prev.facilities,
        }));
      } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          reject(
            new Error(
              "Location permission is required to update a shelter service.",
            ),
          );
        },
      );
  ...
  async function uploadLogo(file) {
    ...
    const filePath = `shelters/${ownerNgoId}/${fileName}`;
  ...
  async function handleSubmit(event) {
    ...
    if (!id) {
      toast.error("Please open this page from a specific shelter record.");
      return;
    }
    ...
    try {
      ...
      if (capacity < preservedOccupied) {
        toast(
          "Capacity is now lower than current occupancy, so availability was set to 0.",
        );
      }
      ...
      const payload = {
        ...
        address: {
          building: form.shelterName?.trim(),
          ...
          {!isLoading ? (
            <div className="ah-page">
              <header className="ah-title">
                <h1>Edit Shelter</h1>
                <p>Update the details to keep this shelter record accurate.</p>
              </header>
                  ...
                  <div className="ah-stack">
                    <FieldGroup label="Facilities">
                      <div className="ah-chip-grid">
                        {DEPARTMENT_OPTIONS.map((facility) => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() => toggleFromList("facilities", facility)}
                            >
                              <i
                                className={`fa-solid ${
                                  active ? "fa-square-check" : "fa-square"
                                }`}
                              />
                              {facility}
                            </button>
                    ...
                    <FieldGroup label="Facility Logo / Cover Image">
                      ...
                      {form.logoUrl ? (
                        <div className="ah-upload-preview">
                          <img src={form.logoUrl} alt="Shelter logo preview" />
                          <button
                            type="button"
